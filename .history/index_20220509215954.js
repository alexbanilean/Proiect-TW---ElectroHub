...
app.get("/produse/:categ?", function(req, res){
    client.query("select * from unnest(enum_range(null::categorie_produs))", function(err, rezCateg){
        ...
        client.query(`select * from produse where ${cond_where}`, function(err, rezQuery){
            // console.log(rezQuery);
            if(!err)
                res.render("pagini/produse", {produse: rezQuery.rows, optiuni : rezCateg.rows, culori: obGlobal.culori, luni: luni, zile: zile});
        });
