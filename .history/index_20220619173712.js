...
app.get("/produse/:categ?", function(req, res){
    client.query("select * from unnest(enum_range(null::categorie_produs))", function(err, rezCateg){
        ...
        var cond_where = req.params.categ ? "categorie=$2" : "1=1";
        ...
        if(req.params.categ){
            ...
            client.query(`select * from produse`, [req.params.categ], function(err, rezQuery){
                if(!err)
                    res.render("pagini/produse", {produse: rezQuery.rows, optiuni : rezCateg.rows, 
                                                    culori: obGlobal.culori, pretMax: obGlobal.pretMax, pretMin : obGlobal.pretMin,
                                                    greutateMin: obGlobal.greutateMin, greutateMax: obGlobal.greutateMax, 
...
app.post("/produse_cos", function(req, res){
    ...
    if(req.body.ids_prod.length != 0){    
        let querySelect = `select nume, descriere, pret, greutate from produse where id in (${req.body.ids_prod.join(",")})`;
