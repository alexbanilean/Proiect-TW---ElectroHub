...
client.query("select distinct culoare from produse", function(err, rezCulori){
    if(!err){
        // console.log(rezCulori.rows);
        obGlobal.culori = rezCulori.rows;
    }
    else
        console.log("eroare culori produse");
});

client.query("select max(pret) from produse", function(err, rezPret){
    if(!err){
        // console.log(rezPret.rows[0].max);
        obGlobal.pretMax = rezPret.rows[0].max;
    }
    else
        console.log("eroare pretMax produse");
});

client.query("select min(pret) from produse", function(err, rezPret){
    if(!err){
        // console.log(rezPret.rows);
        obGlobal.pretMin = rezPret.rows[0].min;
    }
    else
        console.log("eroare pretMin produse");
});

client.query("select max(greutate) from produse", function(err, rezGr){
    if(!err){
        obGlobal.greutateMax = rezGr.rows[0].max;
    }
    else
        console.log("eroare grMax produse");
});

client.query("select min(greutate) from produse", function(err, rezGr){
    if(!err){
        obGlobal.greutateMin = rezGr.rows[0].min;
    }
    else
        console.log("eroare grMin produse");
});
...
stergeAccesari();
setInterval(stergeAccesari, 10 * 60 * 1000);
...
app.get(["/", "/index", "/home"], function(req, res){
    ...
    let queryAccesari = "select nume, username from utilizatori where id in (select distinct user_id from accesari where now() - data_accesare <= interval '5 minutes')";
    client.query(queryAccesari, function(err, rezQuery){
        ...
        if(err)
            ...
            useriOnline = rezQuery.rows;
        
        // console.log(useriOnline);

        res.render("pagini/index", {ip: req.ip, imagini: obImagini.imagini, produse: rezQuery.rows, useriOnline : useriOnline});
    });
})
...
app.get("/produse/:categ?", function(req, res){
    client.query("select * from unnest(enum_range(null::categorie_produs))", function(err, rezCateg){
        ...
        if(req.params.categ){
            client.query(`select * from produse where categorie=$1`, [req.params.categ], function(err, rezQuery){
                if(!err)
                    res.render("pagini/produse", {produse: rezQuery.rows, optiuni : rezCateg.rows, 
                                                    culori: obGlobal.culori, pretMax: obGlobal.pretMax, pretMin : obGlobal.pretMin,
                                                    greutateMin: obGlobal.greutateMin, greutateMax: obGlobal.greutateMax, 
                                                    luni: luni, zile: zile});
            });
        }
        else{
...
app.get("/produs/:id", function(req, res){
    client.query(`select * from produse where id=$1`, [req.params.id], function(err, rezQuery){
        // console.log(rezQuery);
        if(!err)
...
app.post("/produse_cos", function(req, res){
    ...
    if(req.body.ids_prod.length != 0){    
        let querySelect = `select nume, descriere, pret, greutate from produse where id in $1`;
        client.query(querySelect, [req.body.ids_prod.join(",")], function(err, rezQuery){
            if(!err)
...
class ComponentaSimpla extends React.Component{
    ...
    render(){
        return React.createElemenet("p", {title: "text tooltip", className:"test"})
    }
}

window.addEventListener("load", function(){
    let radReact = ReactDOM.createRoot(document.getElementById("test-react"));
    radReact.render(React.createElement(ComponentaSimpla, {proprietate: "ceva"}))
});
