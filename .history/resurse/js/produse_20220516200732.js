...
window.addEventListener("load", function(){
    ...
    // bifare elemente din cos virtual
    ...
    for(let id_prod of idsProduse){
        var ch = document.querySelector(`value=[].select-cos`)
        document.querySelectorAll("[value='4'].select-cos");
    }
    ...
    for(let ch of checkboxes){
        ...
        ch.onchange = function(){
            ...
            if(this.checked){
                idsProduse.push(this.value);
            }
            else{
                var poz = idsProduse.indexOf(this.value);
                if(poz != -1)
                    idsProduse.splice(poz, 1);
            }

            localStorage.setItem("cos_virtual", idsProduse.join(","));
        }
    }
