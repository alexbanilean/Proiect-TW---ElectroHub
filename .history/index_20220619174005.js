            ...
            client.query(queryUtiliz, [campuriText.username], function(err, rezUtiliz){
                if(rezUtiliz.rows.length != 0){
                    ...
                    var comandaInserare = `insert into utilizatori (username, nume, prenume, parola, email, culoare_chat, cod) values 
                                            ('${}', '${campuriText.nume}', '${campuriText.prenume}', '${parolaCriptata}', '${campuriText.email}', '${campuriText.culoare_chat}', '${token}')`;
                    client.query(comandaInserare, [campuriText.username, ], function(err, rezInserare){
                        if(err){
                            console.log(err);
